...
const handleProxyRequest = async (event) => {
    ...
    if (!accessToken || !endpoint) { return { statusCode: 400, body: JSON.stringify({ error: 'Missing parameters.' }) }; }
    
    let apiUrl;
    let headers;

    // --- INICIO DE LA CORRECCIÓN FINAL ---
    // La condición ahora usa 'equipments' (plural)
    if (endpoint.startsWith('equipments') || endpoint.startsWith('equipment')) {
    // --- FIN DE LA CORRECCIÓN FINAL ---
        ...
        headers = { 'Authorization': `Bearer ${accessToken}`, 'Accept': 'application/vnd.deere.axiom.v3+json', 'No_paging': 'True' };
    }
    
    const response = await fetch(apiUrl, { headers });
    const responseBodyText = await response.text();
    if (!response.ok) { console.error(`Error from John Deere API (proxy for ${endpoint}):`, responseBodyText); }
