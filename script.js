...
// No necesitamos una caché global de equipos en esta versión más simple
...
async function handleOrgSelection(orgId) {
    ...
    fetchEquipmentForOrg(orgId); // Llama a la API para los equipos
    fetchFields(orgId);         // Llama a la API para los campos
}

// --- INICIO DE LA CORRECCIÓN ---
// Esta función ahora se llama CADA VEZ que se hace clic en una org
async function fetchEquipmentForOrg(orgId) {
    ...
    try {
        ...
        // Filtra la lista completa usando la lógica correcta
...
// --- El resto del código está bien, lo incluyo para que esté completo ---
...
tabs.addEventListener('click', (e) => { if (!e.target.classList.contains('tab-button')) return; const tabName = e.target.dataset.tab; let contentId = `${tabName}-list`; if (tabName === 'fields') { contentId = 'field-list'; } tabs.querySelectorAll('.tab-button').forEach(btn => btn.classList.remove('active')); e.target.classList.add('active'); document.querySelectorAll('.tab-content').forEach(content => { if (content.id === contentId) { content.classList.add('active'); } else { content.classList.remove('active'); } }); });
