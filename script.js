...
const CLIENT_ID = '0oaqqj19wrudozUJm5d7'; // Movido aquí para ser global
...
async function fetchMachines(orgId) {
    ...
    try {
        ...
        handleApiError(error, machineList, 'máquinas', orgId);
    }
}
...
async function handleApiError(error, container, resourceName, orgId = null) {
    console.error(`Error al obtener ${resourceName}:`, error);

    if (error.status === 403 && orgId) {
        const manageConnectionUrl = `https://connections.deere.com/connections/${CLIENT_ID}/connections-dialog?orgId=${orgId}`;
        const messageHtml = `
            <p><strong>Acceso denegado (403).</strong></p>
            <p>Parece que los permisos para esta organización no están bien configurados. Por favor, haz clic en el botón de abajo para ir directamente a la gestión de permisos.</p>
            <a href="${manageConnectionUrl}" target="_blank" class="permission-link">
                Gestionar Permisos para esta Organización
            </a>
            <p style="margin-top:15px; font-size: 0.9em; color: #718096;">
                En la nueva pestaña, haz clic en "Editar", asigna todos los permisos (Nivel 3), guarda, y luego vuelve aquí y haz clic de nuevo en la organización.
            </p>
        `;
...
function displayOrganizations(organizations) {
    ...
    organizations.forEach(org => {
        ...
        orgItem.dataset.orgId = org.id; // Guardamos el ID aquí
