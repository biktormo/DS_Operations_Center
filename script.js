...
function handleLogin() { /* ... sin cambios ... */ }
...
async function fetchOrganizations() {
    ...
    try {
        ...
        if (hasConnectedOrgs) {
            ...
            const messageHtml = `<h2>Paso Final: Conecta tus Organizaciones</h2><p>Para empezar a ver tus datos, necesitas conectar al menos una de tus organizaciones a SARTOR.</p><a href="${connectionLink.uri}" class="permission-link" style="display:block; text-align:center; margin-top:20px;">Configurar Conexiones</a>`;
            orgList.innerHTML = `<div class="setup-message">${messageHtml}</div>`;
        } else {
            orgList.innerHTML = '<p class="placeholder">No se encontraron organizaciones para conectar.</p>';
        }
    } catch (error) {
...
async function handleOrgSelection(orgId) { /* ... sin cambios ... */ }
async function fetchMachines(orgId) { /* ... sin cambios ... */ }
async function fetchFields(orgId) { /* ... sin cambios ... */ }
async function fetchFieldOperations(fieldId, orgId) { /* ... sin cambios ... */ }

// --- RENDERIZADO Y MANEJO DE UI ---
function handleApiError(error, container, resourceName) { /* ... sin cambios ... */ }
...
function handleApiError(error, container, resourceName) { console.error(`Error al obtener ${resourceName}:`, error); if (error.status === 403) { if (error.body && error.body.links) { const connectionLink = error.body.links.find(link => link.rel === 'connections'); if (connectionLink) { const messageHtml = `Se requieren permisos. <a href="${connectionLink.uri}" target="_blank" class="permission-link">Habilita el acceso aquí</a> y reintenta.`; displayError(messageHtml, container, true); return; } } const generic403Message = `Acceso denegado (403). Es posible que necesites habilitar los permisos ("Connections") para esta organización en el portal de John Deere.`; displayError(generic403Message, container); return; } displayError(`No se pudieron cargar los ${resourceName}. ${error.message}`, container); }
