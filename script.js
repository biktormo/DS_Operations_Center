...
let allEquipment = []; // --- NUEVO: Caché para todos los equipos ---
...
function handleOrgSelection(orgId) {
    ...
    // --- NUEVA LÓGICA: Filtramos en lugar de hacer una nueva llamada ---
    showLoader(machineList, 'Cargando equipos...');
    const equipmentForOrg = allEquipment.filter(eq => eq.organizationId === orgId);
    displayMachines(equipmentForOrg);
    
    fetchFields(orgId);
}

// --- fetchMachines AHORA ES displayMachines y se simplifica ---
function displayMachines(equipment) {
    ...
    equipment.forEach(eq => {
        ...
        // --- Usamos los nuevos nombres de campo: title, principalId ---
...
// (El resto del código es muy similar, pero lo incluyo completo para seguridad)
