...
// --- LÓGICA DE LA API ---
async function fetchWithToken(endpoint) { /* ... sin cambios ... */ }
...
async function getToken(code) { /* ... sin cambios ... */ }
async function fetchOrganizations() { /* ... sin cambios ... */ }
// ... y el resto de funciones ...

// **Para facilitar, aquí está el bloque completo de código para script.js**
...
async function fetchOrganizations() { showLoader(orgList); try { const response = await fetchWithToken('organizations'); const data = await response.json(); const connectionLink = data.links?.find(link => link.rel === 'connections'); if (connectionLink) { dashboard.classList.remove('three-columns'); const messageHtml = `<h2>Paso Final: Conecta tus Organizaciones</h2><p>Para empezar a ver tus datos, necesitas autorizar a cuáles de tus organizaciones puede acceder SARTOR.</p><a href="${connectionLink.uri}" class="permission-link" style="display:block; text-align:center; margin-top:20px;">Configurar Conexiones</a>`; orgList.innerHTML = `<div class="setup-message">${messageHtml}</div>`; } else { dashboard.classList.add('three-columns'); dataPanel.style.display = 'block'; detailsPanel.style.display = 'block'; displayOrganizations(data.values); } } catch (error) { handleApiError(error, orgList, 'organizaciones'); } }
...
function handleApiError(error, container, resourceName) { console.error(`Error al obtener ${resourceName}:`, error); if (error.status === 403) { const connectionUrl = "https://connections.deere.com/connections/add-connection/0oaqqj19wrudozUJm5d7"; const messageHtml = `<p><strong>Acceso denegado (403).</strong></p><p>Este es el paso final. Por favor, haz clic en el botón de abajo para configurar qué organizaciones quieres conectar con SARTOR.</p><a href="${connectionUrl}" target="_blank" class="permission-link">Configurar Conexiones de Organización</a><p style="margin-top:15px; font-size: 0.9em; color: #718096;">Después de guardar los cambios en la nueva pestaña, vuelve aquí y haz clic de nuevo en la organización.</p>`; displayError(messageHtml, container, true); return; } displayError(`No se pudieron cargar los ${resourceName}. ${error.message}`, container); }
